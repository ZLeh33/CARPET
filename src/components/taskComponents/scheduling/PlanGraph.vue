...
const validate = () => {
  const solution = <string>getProperty(dependencies.PlanGraph.validation);
  const expectedEdges = Array.from(solution.matchAll(/\d:out -> \d:in \[.*\]/g)).map((match) => match[0]);

  const userGraph = <string>getProperty(dependencies.PlanGraph.dotDescription);
...
const selectedMethods = getSelectedMethods(
  getProperty(`nodes__${currentNode.value}__components__${props.componentID}__methods`),
  methods
);
</script>
