...
export class Grid {
  ...
  constructor({ stdCanvasH, stdCanvasW, minCellSize, context, canvas }: GridParams) {
  ...
  setLineStyle(line: string | number) {
    if (line === "axis") {
  ...
  setGridLines(direction: "v" | "h", line: number) {
    // Styler
  ...
  setGridValues(direction: string, line: number) {
    ...
    if (direction === "v" && line % 4 === 0 && line !== 0) {
      const value = String(-line / this.ratio);
      const valueOffset = this.context.measureText(value).width + 15;
  ...
  setPan(mouseStartX: number, mouseStartY: number, mousePosX: number, mousePosY: number) {
  ...
  setZoom(zoom: number, mousePosX: number, mousePosY: number) {
    ...
    const beforeX = (mousePosX - this.axisPosX) / this.cellSize / this.ratio;
    const beforeY = (mousePosY - this.axisPosY) / this.cellSize / this.ratio;
    ...
    const afterX = (mousePosX - this.axisPosX) / this.cellSize / this.ratio;
    const afterY = (mousePosY - this.axisPosY) / this.cellSize / this.ratio;
